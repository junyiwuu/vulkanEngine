...
LveDevice::LveDevice(LveWindow &window) : window{window} {
  createInstance();  //initialize the vulkanlibrary and connection between applicaition and vulkan
  setupDebugMessenger();  //sensitive to bug
  createSurface();     //create surface which relies on glfw, vulkan able to display the window result
  pickPhysicalDevice(); //physical device: graphic device in your system, capable of working with vulkan api
  createLogicalDevice(); //describe what features of our physical device we want to use
  createCommandPool(); //set up a command pool for later use, help with command buffer allocation 
}
